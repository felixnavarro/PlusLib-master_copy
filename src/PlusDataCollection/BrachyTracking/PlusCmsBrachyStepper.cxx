...
static PlusCmsBrachyStepper::STEPPERRESPCODE SRC_MOTORIZATION_CODE = "82"; // scale factor (zero if stepper is not motorized)
...
//----------------------------------------------------------------------------
PlusCmsBrachyStepper::PlusCmsBrachyStepper( unsigned long COMPort/*=1*/, unsigned long BaudRate/*=19200*/)
{
...
//----------------------------------------------------------------------------
PlusCmsBrachyStepper::~PlusCmsBrachyStepper()
{
...
//----------------------------------------------------------------------------
PlusStatus PlusCmsBrachyStepper::SetCOMPort(unsigned long COMPort) 
{ 
...
//----------------------------------------------------------------------------
PlusStatus PlusCmsBrachyStepper::SetBaudRate(unsigned long BaudRate)
{ 
  // Change only if we not yet connected
  if (this->m_StepperCOMPort->IsHandleAlive())  
  {
...
//----------------------------------------------------------------------------
PlusStatus PlusCmsBrachyStepper::Connect()
{
  if (!this->m_StepperCOMPort->IsHandleAlive())
  {
...
//----------------------------------------------------------------------------
PlusStatus PlusCmsBrachyStepper::Disconnect()
{
...
//----------------------------------------------------------------------------
PlusStatus PlusCmsBrachyStepper::GetDeviceModelInfo( std::string& version, std::string& model, std::string& serial )
{
...
//----------------------------------------------------------------------------
PlusStatus PlusCmsBrachyStepper::GetVersionInfo(int &iVerHi, int &iVerLo, int &iModelNum, int &iSerialNum)
{
  std::vector<BYTE> vDecodedMessage; 
  PlusStatus retValue(PLUS_FAIL); 
  EnterCriticalSection(&m_CriticalSection); 
...
//----------------------------------------------------------------------------
PlusStatus PlusCmsBrachyStepper::GetReferenceData(STEPPERCOMMAND command, STEPPERRESPCODE respcode, double &count, double &dist, double &scale)
{
  std::vector<BYTE> vDecodedMessage; 
  PlusStatus retValue(PLUS_FAIL); 
  EnterCriticalSection(&m_CriticalSection); 
...
//----------------------------------------------------------------------------
PlusStatus PlusCmsBrachyStepper::GetProbeReferenceData(double &count, double &dist, double &scale)
{  
...
//----------------------------------------------------------------------------
PlusStatus PlusCmsBrachyStepper::GetGridReferenceData(double &count, double &dist, double &scale)
{
  count = 0; dist = 0; scale = 0; 
...
//----------------------------------------------------------------------------
PlusStatus PlusCmsBrachyStepper::GetRotationReferenceData(double &count, double &dist, double &scale)
{
  count = 0; dist = 0; scale = 0; 
...
//----------------------------------------------------------------------------
PlusStatus PlusCmsBrachyStepper::GetEncoderValues(double &PPosition, double &GPosition, double &RPosition, unsigned long &PositionRequestNumber)
{
...
//----------------------------------------------------------------------------
PlusStatus PlusCmsBrachyStepper::GetCalibrationState(int &PState, int &GState, int &RState)
{
  std::vector<BYTE> vDecodedMessage; 
  PlusStatus retValue(PLUS_FAIL); 
  EnterCriticalSection(&m_CriticalSection); 
...
//----------------------------------------------------------------------------
PlusStatus PlusCmsBrachyStepper::IsStepperAlive()
{
  unsigned int Status;
...
//----------------------------------------------------------------------------
PlusStatus PlusCmsBrachyStepper::InitializeStepper(std::string &CalibMsg)
{
  int PState = 0, GState = 0, RState = 0;
  this->GetCalibrationState(PState, GState, RState);
  this->m_IsCalibrated = false;
...
//----------------------------------------------------------------------------
PlusStatus PlusCmsBrachyStepper::ResetStepper()
{
  PlusStatus retValue(PLUS_FAIL); 
  EnterCriticalSection(&m_CriticalSection);
  for ( int i = 0; i<MAXTRIES; i++)
...
//----------------------------------------------------------------------------
PlusStatus PlusCmsBrachyStepper::TurnMotorOn()
{
  PlusStatus retValue(PLUS_FAIL); 
  EnterCriticalSection(&m_CriticalSection);
  for ( int i = 0; i<MAXTRIES; i++)
...
//----------------------------------------------------------------------------
PlusStatus PlusCmsBrachyStepper::TurnMotorOff()
{
  PlusStatus retValue(PLUS_FAIL); 
  EnterCriticalSection(&m_CriticalSection);
  for ( int i = 0; i<MAXTRIES; i++)
...
//----------------------------------------------------------------------------
PlusStatus PlusCmsBrachyStepper::MoveProbeToPosition(double PositionInMm, int &ReturnCode)
{
...
//----------------------------------------------------------------------------
PlusStatus PlusCmsBrachyStepper::StepperButtonEnable()
{
  PlusStatus retValue(PLUS_FAIL); 
  EnterCriticalSection(&m_CriticalSection);
  for ( int i = 0; i<MAXTRIES; i++)
...
//----------------------------------------------------------------------------
PlusStatus PlusCmsBrachyStepper::StepperButtonDisable()
{
  PlusStatus retValue(PLUS_FAIL); 
  EnterCriticalSection(&m_CriticalSection);
  for ( int i = 0; i<MAXTRIES; i++)
...
//----------------------------------------------------------------------------
PlusStatus PlusCmsBrachyStepper::StepperRotateCalibrationEnable()
{
  PlusStatus retValue(PLUS_FAIL); 
  EnterCriticalSection(&m_CriticalSection);
  for ( int i = 0; i<MAXTRIES; i++)
...
//----------------------------------------------------------------------------
PlusStatus PlusCmsBrachyStepper::StepperRotateCalibrationDisable()
{
  PlusStatus retValue(PLUS_FAIL); 
  EnterCriticalSection(&m_CriticalSection);
  for ( int i = 0; i<MAXTRIES; i++)
...
//----------------------------------------------------------------------------
void PlusCmsBrachyStepper::SendStepperCommand(STEPPERCOMMAND command, STEPPERRESPCODE Response, std::vector<BYTE> &vRawMessage)
{
  vRawMessage.clear(); 
...
//----------------------------------------------------------------------------
void PlusCmsBrachyStepper::SendPositionRequestCommand(STEPPERCOMMAND command, std::vector<BYTE> &vRawMessage)
{
  vRawMessage.clear(); 
...
//----------------------------------------------------------------------------
void PlusCmsBrachyStepper::StepperInstruction(STEPPERCOMMAND command)
{
...
//----------------------------------------------------------------------------
void PlusCmsBrachyStepper::ReadStepperAnswer(std::vector<BYTE> &stepperAnswer)
{
  stepperAnswer.clear(); 
  BYTE buff = 0;
...
//----------------------------------------------------------------------------
bool PlusCmsBrachyStepper::IsStepperACKRecieved(std::vector<BYTE> &ackMessage, STEPPERCOMMAND command )
{
...
//----------------------------------------------------------------------------
void PlusCmsBrachyStepper::GetStepperMessage(STEPPERCOMMAND command, const char* Response, std::vector<BYTE> &DecodedMessage )
{
...
//----------------------------------------------------------------------------
void PlusCmsBrachyStepper::SendAckMessage(std::vector<BYTE> StepperMessage)
{
...
//----------------------------------------------------------------------------
void PlusCmsBrachyStepper::GetPositionMessage(std::vector<BYTE> &StepperMessage, std::vector<BYTE> &DecodedMessage)
{
  DecodedMessage.clear(); 
...
//----------------------------------------------------------------------------
void PlusCmsBrachyStepper::DecodeStepperMessage(std::vector<BYTE> StepperMessage, std::vector<BYTE> &DecodedMessage)
{
  DecodedMessage.clear(); 
...
//----------------------------------------------------------------------------
void PlusCmsBrachyStepper::SetScalingParameters()
{
...
//----------------------------------------------------------------------------
void PlusCmsBrachyStepper::ClearBuffer()
{
  BYTE buff;
...
//----------------------------------------------------------------------------
void PlusCmsBrachyStepper::CreateAckMessage(BYTE opcode, std::string *sMessage)
{
  unsigned int ck;
...
//----------------------------------------------------------------------------
unsigned int PlusCmsBrachyStepper::AsciiToBin(unsigned int i, unsigned int j)
{
  i -= 0x30;
  if (i > 9)
  {
...
//----------------------------------------------------------------------------
void PlusCmsBrachyStepper::BinToAscii(unsigned int n, unsigned char *c1, unsigned char *c2)
{                                                                                                                                         
  unsigned int t1,t2;
  t1 = (n/16);
  t2 = (n%16);
